...
#include <set>
#include <random>
#include <ctime> // For seeding with time if desired, or just use a fixed seed for simplicity here
...
vector<Vec3> dotsphere(int density, int seed_val) { // Using seed_val as parameter name
    ...
    std::mt19937 generator; // Modern C++ random number generator

    if (seed_val != -1) {
        generator.seed(seed_val); // Seed with provided value if seed is not -1
    } else {
        std::random_device rd; // For non-deterministic seed
        generator.seed(rd()); // Seed with random device if seed is -1 (or not provided)
    }
    std::uniform_int_distribution<size_t> distribution(0, vertices.size() - 1);
    ...
    if (static_cast<size_t>(density) < vertices.size()) {
        ...
        while (keep.size() < (size_t) density) {
            size_t v = distribution(generator); // Use the generator
            keep.insert(v);
        }
