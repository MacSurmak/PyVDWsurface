...
cdef extern from "vdwsurface.h":
    vector[Vec3] vdw_surface(vector[Vec3] coordinates, vector[string] elements,
                             double scale_factor, double density, int seed) # Added seed to extern declaration

cdef extern from "dotsphere.h": # Add extern for dotsphere with seed
    vector[Vec3] dotsphere(int density, int seed)

def vdwsurface(double[:, ::1] coordinates, elements, double scale_factor=1, double density=1, seed=None): # Added seed to Python function
    ...
    surfpoints = vdw_surface(coordinates_, elements, scale_factor, density, seed if seed is not None else -1) # Pass seed to C++, using -1 as default if None in Python
